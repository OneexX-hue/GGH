...
// Минимальный DDP-клиент (протокол Meteor, на котором построен Rocket.Chat
// realtime API) поверх нативного WebSocket — см. docs/DECISIONS.md,
// "Rocket.Chat клиентская интеграция". Не претендует на полноту протокола,
// реализует только то, что нужно: connect → login(resume) →
// sub(stream-room-messages) → получение новых сообщений.
export class RocketChatRealtimeClient {
  ...
  private sendSubscribe(roomId: string): void {
    ...
    this.ws?.send(
      JSON.stringify({ msg: 'sub', id, name: 'stream-room-messages', params: [roomId, false] }),
    );
  }
