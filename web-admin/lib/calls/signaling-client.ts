...
// docs/DECISIONS.md, "WebRTC-звонки — архитектура"). Обычный browser
// WebSocket — сервер ожидает сообщения формата {event, data}
// (протокол @nestjs/platform-ws), не сырые данные и не socket.io.
...
export class CallsSignalingClient {
  ...
  sendOffer(to: string, callId: string, sdp: RTCSessionDescriptionInit) {
    this.send('offer', { to, callId, sdp });
  }
  sendAnswer(to: string, callId: string, sdp: RTCSessionDescriptionInit) {
    this.send('answer', { to, callId, sdp });
  }
  sendIceCandidate(to: string, callId: string, candidate: RTCIceCandidateInit) {
