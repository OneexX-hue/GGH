...
export function claimQuality(ctx: AppContext, player: Player, body: ClaimQualityRequest): SubmitCodeResponse {
  ...
  if (!acceptsSubmissions(event, now)) {
    ...
    return response('rejected', 'game_not_running', 0, teamTotal(ctx, player.teamId), null);
  }
  ...
  if (!codeRow) {
    ...
    return response('rejected', 'wrong_code', 0, teamTotal(ctx, player.teamId), null);
  }
  ...
  try {
    ...
    return response('rejected', 'already_solved', 0, teamTotal(ctx, player.teamId), null);
  }

  logAttempt(ctx, player, body.taskId, body.code, true, null);
  ctx.hub.publish(event.id, { type: 'scoreboard' });
  return response('accepted', null, codeRow.points, teamTotal(ctx, player.teamId), null);
}
...
function accept(
  ...
  return response('accepted', null, points, teamTotal(ctx, player.teamId), distanceM);
}
...
function reject(
  ...
  return response('rejected', reason, 0, teamTotal(ctx, player.teamId), distanceM);
}
...
function findReplay(ctx: AppContext, teamId: string, idempotencyKey: string): SubmitCodeResponse | null {
  const row = ctx.db
...
function findQualityReplay(ctx: AppContext, teamId: string, idempotencyKey: string): SubmitCodeResponse | null {
  const row = ctx.db
    .prepare(
    ...
    .get(teamId, idempotencyKey) as Row | undefined;
  if (!row) return null;
  return response('accepted', null, Number(row['points']), teamTotal(ctx, teamId), null);
}
...
function response(
  ...
  return { status, reason, pointsAwarded, totalPoints, distanceM };
}
